...
{/* input: 待比较的点坐标， 鼠标点击坐标，判定宽度
	output: flag=true: 选中控制点
	*/
	bool flag = true;
...
void CacuSpline(CanvasData* data)
{
	...
	if (d < 3)
		return;
	Eigen::SparseMatrix<float> A(d - 2, d - 2);//系数矩阵
	...
	//解三对角方程
	A.setFromTriplets(tv.begin(), tv.end());//给矩阵A赋值
